...
public class SnippetController {
	...
	public RedirectView addNewSnippet(@RequestParam("snippet_name") String name,
			@RequestParam("snippet_language") String language,
			@RequestParam("snippet_accessibility") String accessibility) {

		User currentUser = getCurrentUser();

		CodeSnippet snippet = snippetService.createNewSnippet(currentUser, name,
				languageService.findByLanguage(language), accessibility);
	...
	public ModelAndView saveSnippet(@RequestParam("snippet_id") Integer id, @RequestParam("snippet_name") String name,
		...
		CodeSnippet targetSnippet = snippetService.findById(id);
		
		// change it only if it's the owner
		String updateAccessibility = targetSnippet.getAccessibility();
		if(checkSnippetIsOwner(targetSnippet))
			updateAccessibility = access; 
		
		snippetService.updateSnippet(id, name, content, userService.findUserById(user),
				languageService.findByLanguage(language), updateAccessibility);
	...
	public ModelAndView getEditorSnippetForm(@PathVariable(value = "id") Integer id) {
		...
		if (snippet == null || !checkSnippetAccessible(snippet))
	...
	@ResponseBody
	public String executeSnippet(@PathVariable(value = "id") Integer id) {
		CodeSnippet codeSnippet = snippetService.findById(id);
		String jsonOutput = "";

		if (codeSnippet != null && checkSnippetAccessible(codeSnippet))
			jsonOutput = snippetExecutor.runSnippet(codeSnippet);
		return jsonOutput;
	}

	private boolean checkSnippetAccessible(CodeSnippet targetSnippet) {
		return targetSnippet.getAccessibility().equals("Public")
				|| checkSnippetIsOwner(targetSnippet);
	}
	
	private boolean checkSnippetIsOwner(CodeSnippet targetSnippet)
	{
		return targetSnippet.getOwner().getId().equals(getCurrentUser().getId());
	}
